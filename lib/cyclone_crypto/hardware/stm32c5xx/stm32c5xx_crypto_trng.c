...
//Global variable
static hal_rng_handle_t rng_handle;
...
error_t trngInit(void)
{
   ...
   status = HAL_RNG_Init(&rng_handle, HAL_RNG);
...
error_t trngGetRandomData(uint8_t *data, size_t length)
{
   ...
   for(i = 0; i < length; i++)
   {
      ...
      if((i % 4) == 0)
      {
         ...
         status = HAL_RNG_GenerateRandomNumber(&rng_handle, &value, 1,
            HAL_MAX_DELAY);
         //Check status code
         if(status != HAL_OK)
         {
