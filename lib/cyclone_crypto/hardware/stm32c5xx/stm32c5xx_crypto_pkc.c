/**
 * @file stm32c5xx_crypto_pkc.c
 * @brief STM32C5 public-key hardware accelerator (PKA)
 *
 * @section License
 *
...
#include "hardware/stm32c5xx/stm32c5xx_crypto_pkc.h"
#include "pkc/rsa.h"
#include "ecc/ec.h"
...
#if (STM32C5XX_CRYPTO_PKC_SUPPORT == ENABLED)
...
error_t pkaExportMpi(Mpi *dest, uint_t length, uint_t offset)
{
   ...
   while(length > 0 && PKA_RAM[offset + length - 1] == 0)
   {
      length--;
   }
...
error_t mpiExpMod(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p)
{
   ...
   if(modLen <= PKA_MAX_ROS && expLen <= PKA_MAX_ROS)
   {
      ...
      if(!error)
      {
         ...
         PKA_RAM[PKA_MODULAR_EXP_IN_OP_NB_BITS] = modLen;
         PKA_RAM[PKA_MODULAR_EXP_IN_OP_NB_BITS + 1] = 0;

         //Specify the length of the exponent, in bits
         PKA_RAM[PKA_MODULAR_EXP_IN_EXP_NB_BITS] = expLen;
         PKA_RAM[PKA_MODULAR_EXP_IN_EXP_NB_BITS + 1] = 0;
...
error_t pkaRsaCrtExp(const RsaPrivateKey *key, const Mpi *c, Mpi *m)
{
      ...
      dpLen <= (nLen / 2) && dqLen <= (nLen / 2) && qinvLen <= (nLen / 2))
   {
      ...
      //Specify the length of the operand, in bits
      PKA_RAM[PKA_RSA_CRT_EXP_IN_MOD_NB_BITS] = nLen;
      PKA_RAM[PKA_RSA_CRT_EXP_IN_MOD_NB_BITS + 1] = 0;
   ...
   const EcPoint3 *s)
{
   ...
   if(modLen <= PKA_MAX_EOS && orderLen <= PKA_MAX_EOS)
   {
      ...
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_OP_NB_BITS] = modLen;
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_OP_NB_BITS + 1] = 0;

      //Specify the length of the scalar, in bits
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_EXP_NB_BITS] = orderLen;
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_EXP_NB_BITS + 1] = 0;

      //Set the sign of the coefficient A
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_A_COEFF_SIGN] = 0;
      PKA_RAM[PKA_ECC_SCALAR_MUL_IN_A_COEFF_SIGN + 1] = 0;
      ...
      //Clear error code
      PKA_RAM[PKA_ECC_SCALAR_MUL_OUT_ERROR] = PKA_STATUS_INVALID;
      ...
      if(PKA_RAM[PKA_ECC_SCALAR_MUL_OUT_ERROR] == PKA_STATUS_SUCCESS)
      {
         error = NO_ERROR;
      }
      else
   ...
   EcdsaSignature *signature)
{
   ...
   if(!error)
   {
      ...
      //Specify the length of the modulus, in bits
      PKA_RAM[PKA_ECDSA_SIGN_IN_MOD_NB_BITS] = modLen;
      PKA_RAM[PKA_ECDSA_SIGN_IN_MOD_NB_BITS + 1] = 0;

      //Specify the length of the base point order, in bits
      PKA_RAM[PKA_ECDSA_SIGN_IN_ORDER_NB_BITS] = orderLen;
      PKA_RAM[PKA_ECDSA_SIGN_IN_ORDER_NB_BITS + 1] = 0;

      //Set the sign of the coefficient A
      PKA_RAM[PKA_ECDSA_SIGN_IN_A_COEFF_SIGN] = 0;
      PKA_RAM[PKA_ECDSA_SIGN_IN_A_COEFF_SIGN + 1] = 0;
      ...
      //Clear error code
      PKA_RAM[PKA_ECDSA_SIGN_OUT_ERROR] = PKA_STATUS_INVALID;
      ...
      //Successful computation?
      if(PKA_RAM[PKA_ECDSA_SIGN_OUT_ERROR] == PKA_STATUS_SUCCESS)
      {
         error = NO_ERROR;
      }
      else
   ...
   const uint8_t *digest, size_t digestLen, const EcdsaSignature *signature)
{
   ...
   //Specify the length of the modulus, in bits
   PKA_RAM[PKA_ECDSA_VERIF_IN_MOD_NB_BITS] = modLen;
   PKA_RAM[PKA_ECDSA_VERIF_IN_MOD_NB_BITS + 1] = 0;

   //Specify the length of the base point order, in bits
   PKA_RAM[PKA_ECDSA_VERIF_IN_ORDER_NB_BITS] = orderLen;
   PKA_RAM[PKA_ECDSA_VERIF_IN_ORDER_NB_BITS + 1] = 0;

   //Set the sign of the coefficient A
   PKA_RAM[PKA_ECDSA_VERIF_IN_A_COEFF_SIGN] = 0;
   PKA_RAM[PKA_ECDSA_VERIF_IN_A_COEFF_SIGN + 1] = 0;
   ...
   //Clear result
   PKA_RAM[PKA_ECDSA_VERIF_OUT_RESULT] = PKA_STATUS_INVALID;
   ...
   if(PKA_RAM[PKA_ECDSA_VERIF_OUT_RESULT] == PKA_STATUS_SUCCESS)
   {
      error = NO_ERROR;
   }
   else
